...
export const SingleLine = ({ text, active }: DescriptionProps) => {
  return (
    <p className="flex items-center justify-center gap-5">
      ...
      <span
        className={cn("text-[#EEEEEE]", active && "text-active font-medium")}
      >
        {text}
      </span>
    </p>
  ...
  return (
    <div>
      {textArray?.map((text, index) => (
        ...
        >
          <span>
            {activeIndex === index ? <YellowBullet /> : <WhiteBullet />}
          </span>
          <span
            className={cn(activeIndex === index && "text-active font-medium")}
          >
            {text}
          </span>
        </p>
